...
internal static class IVehicleDebug
{
    ...
    public static void DisplayDebugInfo(this IVehicle self, Terrain terrain = null, [CallerFilePath] string f = null, [CallerMemberName] string n = null)
    {
        ...
        dbg.Add(" - WheelCenter", () => $"{wheelCenter.Rounded(3)} ({terrain?.Altitude(body.ToGlobal(wheelCenter), 3)}m)", f: f, n: n);
        dbg.Add(" - GravityCenter", () => $"{body?.CenterOfMass.Rounded(3)} ({terrain?.Altitude(body.ToGlobal(body.CenterOfMass), 3)}m)", f: f, n: n);
        dbg.Add(" - VehiclePosition", () => $"{self.GlobalPosition.Rounded(3)} ({terrain?.Altitude(body.GlobalPosition, 3)}m)", f: f, n: n);
